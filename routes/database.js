...
module.exports = {
  ...
  getTop5: function(req,res){
    ...
    var fs = require('fs')
      ...
      connection.query("SELECT users.name,SUM(scores.scores) from scores inner join users on scores.id = users.id group by scores.id order by scores.scores desc", function (err, result, fields) {
        if (err) throw err;
        console.log(result);
